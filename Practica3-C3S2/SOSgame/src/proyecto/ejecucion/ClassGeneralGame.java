...
public class ClassGeneralGame extends ClassScore {
    ...
    public void resultGeneralGameLogico() {
        ...
        private void generalGame(String letter, int fil, int col, int[][] info, ClassPlayer currentPlayer) {
            if (currentPlayer.getPlayer().equals("BLUE")) {
                setScorePlayer2(getScorePlayer2() + scoreSOS(letter, fil, col, info));
            } else if (currentPlayer.getPlayer().equals("RED")) {
                setScorePlayer1(getScorePlayer1() + scoreSOS(letter, fil, col, info));
            }
            if (fullScreen(info)) {
